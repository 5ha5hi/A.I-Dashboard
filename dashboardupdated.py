...
def fetch_db_schema(conn, database):
    query = """
...
def run_sql_query(conn, sql):
    if not any(sql.lstrip().lower().startswith(k) for k in ("select", "with")):
...
def generate_dashboard(df: pd.DataFrame):
    ...
    if end < len(chart_defs):
        st.button(
    "➕ Show More Charts",
    key="show_more_charts",
    on_click=next_dashboard_page
)
...
# ======================================================
# 🤖 Natural Language → SQL (DB Mode)
# ======================================================
def generate_sql(schema: dict, user_input: str) -> str:
    prompt = f"""
...
2. Do NOT use INSERT, UPDATE, DELETE, DROP, ALTER
3. Use only tables and columns from the schema
4. Add LIMIT 100 if result may be large
5. Return ONLY SQL
...
with st.sidebar:
    ...
    if st.session_state.data_source == "CSV":
        st.session_state.pop("active_table", None)
    ...
    if st.session_state.data_source == "Database":
        ...
        if st.button("Connect to Database"):
            conn = get_mysql_connection(host, user, password, database, port)
            if conn:
                st.session_state.db_conn = conn
                st.session_state.db_schema = fetch_db_schema(conn, database)
...
# ======================================================
# 📋 Table Selector (Database Mode)
# ======================================================
if (
    st.session_state.data_source == "Database"
    and st.session_state.db_schema
):
    st.markdown("### 📋 Select Table")

    table_names = sorted(st.session_state.db_schema.keys())
    ...
    if selected_table != "-- Select a table --":
        try:
            if st.session_state.get("active_table") != selected_table:
                query = f"SELECT * FROM `{selected_table}` LIMIT 100"
                df = run_sql_query(st.session_state.db_conn, query)

                st.session_state.df = df
                st.session_state.original_df = df.copy()
                st.session_state.file_name = f"mysql:{selected_table}"

            # Reset downstream state ONLY when table changes
                st.session_state.last_run_result = None
                st.session_state.last_generated_code = None
                st.session_state.last_user_input = ""
                st.session_state.error_occurred = False
...
if st.session_state.df is not None:
    ...
    st.info("📄 No data loaded yet. Upload a CSV or select a database table.")
    ...
    if user_input.strip():
        ...
        try:
            ...
            if st.session_state.data_source == "CSV":
                ...
                sql = generate_sql(st.session_state.db_schema, user_input)
                st.session_state.last_generated_code = sql
                st.session_state.last_user_input = user_input

                df = run_sql_query(st.session_state.db_conn, sql)
                st.session_state.df = df
                st.session_state.last_run_result = df
...
if st.session_state.last_user_input:
    ...
    if st.session_state.last_generated_code:
        with st.expander("🤖 View AI Generated Code", expanded=False):
            lang = "sql" if st.session_state.data_source == "Database" else "python"
            st.code(st.session_state.last_generated_code, language=lang)
